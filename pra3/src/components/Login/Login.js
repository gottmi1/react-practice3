...
const Login = (props) => {
  ...
  // 객체 디스트럭처링 구문의 별칭할당. 값만 변경되고 유효성은 변경되지 않으면 이 이펙트는 다시 실행되지 않는다.
  // State들의 isValid가 이미 true일 떄 입력을 더 받더라도 쓸데없는 유효성 검사를 더 하지 않아도 되도록
  ...
  const passwordChangeHandler = (event) => {
    ...
    // setFormIsValid(emailState.isValid && event.target.value.trim().length > 6); // isValid가 true일 때
  };
